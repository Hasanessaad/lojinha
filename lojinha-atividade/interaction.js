...
for(let i=0; i <  ITENS_LOJA.length ;i++){
    ...
    let nomee = document.createElement('p');
    let price = document.createElement('p');
    let description = document.createElement('p');
    
    img.src = ITENS_LOJA[i].url_img;
    nomee.innerText = ITENS_LOJA[i].nome;
    price.innerText = ITENS_LOJA[i].preco;
    description.innerText = ITENS_LOJA[i].descricao;

    allfather.appendChild(box);
    box.appendChild(img);
    box.appendChild(nomee);
